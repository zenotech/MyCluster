...
def get_stats_time(stats):
    ...
    if cputime_delta and wallclock_delta:
        time_ratio = (
            float(cputime_delta.total_seconds()) / wallclock_delta.total_seconds()
        )
    ...
    print(
        (
            "User name: {0} {1}".format(
                job_db.user_db["user"].first_name, job_db.user_db["user"].last_name
            )
        )
    )
    ...
    for i, j in enumerate(jobs):
        ...
        if time_ratio:
            try:
                efficiency = old_div(
                    time_ratio,
                    (int(jobs[j].num_tasks) * int(jobs[j].threads_per_task)) * 100.0,
                )
