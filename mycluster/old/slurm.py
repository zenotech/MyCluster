...
def submit(script_name, immediate, depends_on=None, depends_on_always_run=False):
    ...
    if not immediate:
        if depends_on and depends_on_always_run:
            with os.popen(
                "sbatch %s --kill-on-invalid-dep=yes --dependency=afterany:%s %s"
                % (additional_cmd, depends_on, script_name)
            ) as f:
                output = f.readline()
                try:
            ...
            with os.popen(
                "sbatch %s --kill-on-invalid-dep=yes --dependency=afterok:%s %s"
                % (additional_cmd, depends_on, script_name)
            ) as f:
                output = f.readline()
                try:
...
def job_stats(job_id):
    stats_dict = {}
    with os.popen(
        "sacct --noheader --format JobId,Elapsed,TotalCPU,Partition,NTasks,AveRSS,State,ExitCode -P -j "
        + str(job_id)
    ) as f:
        try:
            line = f.readline()
...
def job_stats_enhanced(job_id):
    ...
    stats_dict = {}
    with os.popen(
        "sacct --noheader --format JobId,Elapsed,TotalCPU,Partition,NTasks,AveRSS,State,ExitCode,start,end -P -j "
        + str(job_id)
    ) as f:
        try:
            line = f.readline()
...
def running_stats(job_id):
    ...
    with os.popen(
        "sstat --noheader --format AveCPU,AveRSS,NTasks -j " + str(job_id)
    ) as f:
        try:
            line = f.readline()
            new_line = re.sub(" +", " ", line.strip())
